-- this is the simple logic of gyro-inductive compass
size = {2048, 2048}
...
defineProperty("GIK_sw", globalProperty("an-24/gauges/GIK_sw"))

-- control
defineProperty("GIK_button", globalProperty("an-24/gauges/GIK_button"))
defineProperty("GIK_curse", globalProperty("an-24/gauges/GIK_curse"))
defineProperty("ap_GIK_curse", globalProperty("an-24/ap/curse_gik"))

-- sources
defineProperty("gyro", globalProperty("an-24/gauges/gyro_curse"))
...
defineProperty("real_mag_curse", globalProperty("sim/flightmodel/position/mag_psi")) -- XP12: magpsi renamed to mag_psi
defineProperty("turn", globalProperty("sim/flightmodel/misc/turnrate_roll")) -- turn rate deg/sec. must be used with some coef.

defineProperty("SC_master", globalProperty("scp/api/ismaster")) -- status of SmartCopilot
defineProperty("sc_angle", globalProperty("an-24/gauges/sc_angle")) -- SmartCopilot
defineProperty("sc_ap_angle", globalProperty("an-24/gauges/sc_ap_angle")) -- SmartCopilot
...
registerCommandHandler(findCommand("sim/autopilot/heading"), 0, function(p)
    if p == 1 then
        set(GIK_button, 1)
    else
        set(GIK_button, 0)
...
local angle_correct = 0 -- correction to make angle closer to real magnetic heading
...
function update()
    ...
    if passed > 0 then
        ...
        -- check power
        local power = dcOK() and acOK() and get(GIK_sw) == 1
        ...
        if mag_curse > 180 then
            mag_curse = mag_curse - 360
        elseif mag_curse < -180 then
            mag_curse = mag_curse + 360
        end
        if power and math.abs(angle - mag_curse) > 1 then
            ...
            if angle_correct > 180 then
                angle_correct = angle_correct - 360
            elseif angle_correct < -180 then
                angle_correct = angle_correct + 360
            end
        end
        ...
        -- angle correction for autopilot
        ...
        if get(SC_master) == 1 then
            ...
            if power and math.abs(ap_angle - mag_curse) > 1 then
                ...
                if ap_angle_correct > 180 then
                    ap_angle_correct = ap_angle_correct - 360
                elseif ap_angle_correct < -180 then
                    ap_angle_correct = ap_angle_correct + 360
                end
            end
