...
local function anim(command_up, command_down, dataref, angle)
    local drf = gPi(dataref) -- created in glbl_drfs.lua
    -- For all commands phase (p) equals: 0 on press; 1 while holding; 2 on release
    registerCommandHandler(findCommand(command_up), 0, function(p)
        if p == 0 then
            set(drf, (get(drf) + angle) % 360)
        end
        return 0
    end)
    registerCommandHandler(findCommand(command_down), 0, function(p)
        if p == 0 then
            set(drf, (get(drf) - angle) % 360)
        end
        return 0
