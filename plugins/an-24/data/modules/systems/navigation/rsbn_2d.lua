...
defineProperty("detailed", sasl.gl.loadImage("rsbn_e.dds", 0, 143, 150, 150))
defineProperty("glass_cap", sasl.gl.loadImage("rsbn_e.dds", 11, 97, 78, 32))
defineProperty("needle2", sasl.gl.loadImage("rsbn_e.dds", 205, 92, 22, 158))
...
defineProperty("needle1", langImage("needles", 311, 10, 18, 173))
...
function update()
    ...
    if power == 1 and get(rsbn_receive) == 1 then
        v1 = azimut + (math.random() - 0.49999) * 2
    elseif power == 1 then
        v1 = v1 + (math.random() - 0.3) * 2
    end
    ...
    if power == 1 and get(rsbn_receive) == 1 then
        v2 = azimut2 + (math.random() - 0.49999) * 2
    elseif power == 1 then
        v2 = v2 + (math.random() - 0.3) * 2
    end
    ...
    if passed > 0 then
        ...
        if delta3 > 180 then
            delta3 = delta3 - 360
        elseif delta3 < -180 then
            delta3 = delta3 + 360
        end
        angle3 = angle3 + 15 * delta3 * passed
        if angle3 > 180 then
            angle3 = angle3 - 360
        elseif angle3 < -180 then
            angle3 = angle3 + 360
        end
        ...
        if delta4 > 180 then
            delta4 = delta4 - 360
        elseif delta4 < -180 then
            delta4 = delta4 + 360
        end
        angle4 = angle4 + 15 * delta4 * passed
        if angle4 > 180 then
            angle4 = angle4 - 360
        elseif angle4 < -180 then
            angle4 = angle4 + 360
        end
        ...
        if delta5 > 180 then
            delta5 = delta5 - 360
        elseif delta5 < -180 then
            delta5 = delta5 + 360
        end
        angle5 = angle5 + 15 * delta5 * passed
        if angle5 > 180 then
            angle5 = angle5 - 360
        elseif angle5 < -180 then
            angle5 = angle5 + 360
        end
    end
    ...
    if power27 and acOK() and power_sw == 1 then
        power = 1
    else
        power = 0
    end
    ...
    if get(rsbn_receive) == 1 and power == 1 then
        ...
        azimut = math.mod(math.atan2(math.sin(lon1 - lon2) * math.cos(lat2), math.cos(lat1) * math.sin(lat2) -
            math.sin(lat1) * math.cos(lat2) * math.cos(lon1 - lon2)) + math.pi, 2 * math.pi)
        ...
        elseif mode == 3 or mode == 4 then
            ...
            -- SRP
        elseif mode == 5 then
            lat3 = math.asin(math.sin(lat2) * math.cos(set_targetdist / 1850 * math.pi / 10800) + math.cos(lat2) *
                                 math.sin(set_targetdist / 1850 * math.pi / 10800) *
                                 math.cos(set_targetangle * math.pi / 180))
            if (math.cos(lat3) == 0) then
                lon3 = lon2
            else
                lon3 = math.mod(lon2 -
                                    math.asin(
                        math.sin(set_targetangle * math.pi / 180) * math.sin(set_targetdist / 1850 * math.pi / 10800) /
                            math.cos(lat3)) + math.pi, 2 * math.pi) - math.pi
            end
            SRP_azimut = math.mod(math.atan2(math.sin(lon1 - lon3) * math.cos(lat3), math.cos(lat1) * math.sin(lat3) -
                math.sin(lat1) * math.cos(lat3) * math.cos(lon1 - lon3)), 2 * math.pi)
            SRP_orbit = (2 * math.asin(math.sqrt(
                (math.sin((lat1 - lat3) / 2)) ^ 2 + math.cos(lat1) * math.cos(lat3) * (math.sin((lon1 - lon3) / 2)) ^ 2)))
        ...
        if (set_azimut - azimut) < 1.1 and (set_azimut - azimut) > -1.1 and (orbit - set_orbit) < 1100 and
            (orbit - set_orbit) > -1100 then
            red_lamp = 1
        else
            red_lamp = 0
        end
...
components = {
    ...
    texture {
        ...
        image = langImage("rsbn", 0, 300, 1024, 724),
        visible = true
    }, 
