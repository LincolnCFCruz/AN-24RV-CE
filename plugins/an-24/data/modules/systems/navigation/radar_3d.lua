...
defineProperty("turn_sw", globalProperty("sim/cockpit2/switches/custom_slider_on[15]")) -- switch to rotate RLS
defineProperty("deg1", globalProperty("sim/flightmodel/position/psi")) -- acf mag heading
defineProperty("deg2", globalProperty("sim/flightmodel/position/hpath")) -- real mag heading

defineProperty("rls_power_sw", globalProperty("an-24/rls/rls_power_sw")) -- power switch
defineProperty("rls_power_cc", globalProperty("an-24/rls/rls_power_cc")) -- power switch
...
-- V11/XP12.3+ FIX: EFIS_weather_on no longer activates the weather radar by
-- itself! EFIS_weather_mode is required: 0=OFF, 1=TEST, 2=WX, 3=WX+T, 4=MAP,
-- 5=TURB. On a cold start it defaults to 0 -> black screen.
defineProperty("EFIS_weather_mode", globalProperty("sim/cockpit2/EFIS/EFIS_weather_mode"))
...
-- images
defineProperty("mask1", sasl.gl.loadImage("radar_mask1.png", 0, 0, 256, 256))
defineProperty("mask2", sasl.gl.loadImage("radar_mask2.png", 0, 0, 256, 256))
defineProperty("scale", sasl.gl.loadImage("radar_scale.png", 0, 76, 256, 180))

defineProperty("needle_1", sasl.gl.loadImage("radar_scale.png", 0, 62, 256, 4))
defineProperty("needle_2", sasl.gl.loadImage("radar_scale.png", 0, 52, 256, 4))
defineProperty("needle_3", sasl.gl.loadImage("radar_scale.png", 0, 42, 256, 4))
...
function update()
    ...
    -- V11/XP12.3+ FIX: explicitly enable the weather radar via the new dataref.
    -- 3 = WX+T (weather + turbulence). Fixes the black screen on cold start —
    -- without this the radar only worked on a hot start.
    set(EFIS_weather_mode, 3)
    set(EFIS_tcas_on, 0)
    ...
    local power_el = dcOK() and ac36OK() or acOK()
...
components = { 
	...
	needleLit {
		...
		image = function()
			local a
			if needle_show == 1 then
				a = get(needle_1)
			elseif needle_show == 2 then
				a = get(needle_2)
			else
				a = get(needle_3)
			end
