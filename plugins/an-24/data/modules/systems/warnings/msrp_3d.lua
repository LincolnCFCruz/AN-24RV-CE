...
defineProperty("utc_time", globalProperty("sim/time/zulu_time_sec")) -- sim UTC time
defineProperty("msl_alt", globalProperty("sim/flightmodel/position/elevation")) -- barometric alt in meters.
defineProperty("radio_alt", globalProperty("sim/cockpit2/gauges/indicators/radio_altimeter_height_ft_pilot")) -- altitude, measured by gauge
...
defineProperty("elevator", globalProperty("sim/flightmodel2/wing/elevator1_deg[0]")) -- elevator position deg. negative = UP
defineProperty("rudder", globalProperty("sim/flightmodel2/wing/rudder1_deg[0]")) -- rudder position deg. negative = right
defineProperty("aileron", globalProperty("sim/flightmodel2/wing/aileron1_deg[1]")) -- right aileron position. negative deg - right roll
-- V11/XP12 FIX: magpsi is deprecated ("DO NOT USE THIS" per the official docs)
-- and returns a wrong value in XP12 — replaced by mag_psi.
defineProperty("course", globalProperty("sim/flightmodel/position/mag_psi")) -- magnetic course
defineProperty("ikm_L", globalProperty("an-24/gauges/torque_left")) -- torque left
defineProperty("ikm_R", globalProperty("an-24/gauges/torque_left")) -- torque right
defineProperty("throttle1", globalProperty("an-24/misc/virt_rud1"))
defineProperty("throttle2", globalProperty("an-24/misc/virt_rud2"))
defineProperty("roll", globalProperty("an-24/ap/indicated_roll")) -- roll for autopilot
defineProperty("pitch", globalProperty("an-24/ap/indicated_pitch")) -- pitch for autopilot
defineProperty("flap_deg1", globalProperty("sim/flightmodel2/wing/flap1_deg[0]")) -- left flap deg

defineProperty("latitude", globalProperty("sim/flightmodel/position/latitude")) -- degrees	The latitude of the aircraft
defineProperty("longitude", globalProperty("sim/flightmodel/position/longitude")) -- degrees	The longitude of the aircraft
...
defineProperty("fire_warinig", globalProperty("an-24/fire/fire_warinig")) -- fire warning for siren logic
defineProperty("skv_siren", globalProperty("an-24/skv/skv_siren")) -- depressurisation
defineProperty("thrust_L", globalProperty("sim/cockpit2/engine/indicators/thrust_n[0]")) -- engine thrust
defineProperty("thrust_R", globalProperty("sim/cockpit2/engine/indicators/thrust_n[1]")) -- engine thrust
defineProperty("pitch_stop_set", globalProperty("an-24/prop/pitch_stop_set")) -- set up pitch mid stop
defineProperty("ap_on", globalProperty("an-24/ap/ap_on_lit")) -- AP engaged light
defineProperty("high_vibro", globalProperty("an-24/gauges/high_vibro")) -- high vibration
...
defineProperty("gear_down", globalProperty("an-24/gauges/gear_down")) -- all gears down
defineProperty("roll_high", globalProperty("an-24/gauges/roll_high")) -- too much roll
...
local filename = blackBoxDir .. "/default_file.txt" -- current name of file
...
function create_file()
    ...
    filename = blackBoxDir .. "/" .. now["year"] .. "_" .. now["month"] .. "_" .. now["day"] .. "_" .. now["hour"] ..
                   "_" .. now["min"] .. ".txt"
    ...
    if savefile then
        ...
        print("error saving file. check if there is \"black_box\" folder here: " .. blackBoxDir ..
...
function write_file() -- write parameters to file
    ...
    if savefile then
        ...
        -- single signals
        if get(ice_on_plane) == 1 then
            savefile:write("icing,  ")
        end
        if get(fire_warinig) == 1 then
            savefile:write("fire,  ")
        end
        if get(skv_siren) == 1 then
            savefile:write("cab_alt,  ")
        end
        if get(thrust_L) < 0 then
            savefile:write("eng_L_neg,  ")
        end
        if get(thrust_R) < 0 then
            savefile:write("eng_R_neg,  ")
        end
        if get(pitch_stop_set) == 0 then
            savefile:write("props_nostop,  ")
        end
        if get(ap_on) == 1 then
            savefile:write("AP_ON,  ")
        end
        if get(high_vibro) == 1 then
            savefile:write("eng_stress,  ")
        end
        if get(feather1_button) == 1 or get(feather2_button) == 1 then
            savefile:write("prop_feather,  ")
        end
        if get(outer_marker) == 1 then
            savefile:write("Out_mark,  ")
        end
        if get(middle_marker) == 1 then
            savefile:write("Mid_mark,  ")
        end
        if get(inner_marker) == 1 then
            savefile:write("In_mark,  ")
        end
        if get(gear_down) == 1 then
            savefile:write("LG_down,  ")
        end
        if get(roll_high) == 1 then
            savefile:write("roll_high,  ")
        end
        if get(SSOS_alarm) == 1 then
            savefile:write("TAWS,  ")
        end
...
local created = false -- flag for successfully created file
local started = false -- flag for started logic and create file once
...
function update()
    ...
    -- check the power
    power = get(black_box) == 1 and get(bus_DC_27_volt) > 20 and (get(ias) * 1.852 > 70 or get(gear2_deflect) < 0.05) -- black box working when there is power and airplane is flying
