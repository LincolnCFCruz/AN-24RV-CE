...
defineProperty("fire_valve1", globalProperty("an-24/fuel/fire_valve1")) -- fire valve for engine 1
defineProperty("fire_valve2", globalProperty("an-24/fuel/fire_valve2")) -- fire valve for engine 2
defineProperty("fire_valve3", globalProperty("an-24/fuel/fire_valve3")) -- fire valve for engine 3
...
defineProperty("auto_ff", globalProperty("an-24/fuel/auto_ff")) -- automatic fuel flow
defineProperty("fuel_q_meter1", globalProperty("an-24/fuel/q_meter1_switch")) -- switcher for fuel meter 1
defineProperty("fuel_q_meter2", globalProperty("an-24/fuel/q_meter2_switch")) -- switcher for fuel meter 2
defineProperty("ff_meter", globalProperty("an-24/fuel/ff_meter_switch")) -- switcher for fuel flow meter 1
...
defineProperty("fuel_q_2", globalProperty("sim/flightmodel/weight/m_fuel[1]")) -- fuel quantity for tank 2
defineProperty("fuel_q_3", globalProperty("sim/flightmodel/weight/m_fuel[2]")) -- fuel quantity for tank 3
defineProperty("fuel_q_4", globalProperty("sim/flightmodel/weight/m_fuel[3]")) -- fuel quantity for tank 4
defineProperty("fuel_q_all", globalProperty("sim/flightmodel/weight/m_fuel_total")) -- fuel quantity for all tanks

-- failures
defineProperty("fuel_block_1", globalProperty("sim/operation/failures/rel_fuel_block0")) -- fuel filter block
defineProperty("fuel_block_2", globalProperty("sim/operation/failures/rel_fuel_block1"))
defineProperty("fuel_block_3", globalProperty("sim/operation/failures/rel_fuel_block2"))
defineProperty("fuel_block_4", globalProperty("sim/operation/failures/rel_fuel_block3"))

defineProperty("fuel_fluct1", globalProperty("sim/operation/failures/rel_fuelfl0")) -- fuel fluctuation engine 1
defineProperty("fuel_fluct2", globalProperty("sim/operation/failures/rel_fuelfl1")) -- fuel fluctuation engine 1
defineProperty("fuel_fluct3", globalProperty("sim/operation/failures/rel_fuelfl2")) -- fuel fluctuation engine 2

-- XP12 REAL FUEL METER (unusable residual):
-- Real An-24 has ~50 kg unusable residual in each tank (200 kg total).
-- This residual is physically in the tanks but unavailable to engines. The fuel meter
-- SHOWS LESS than actual quantity — pilot accounts for this in calculations.
-- If switch real_fuel_meter = 0 — fuel meter shows exactly.
defineProperty("set_real_fuel_meter", globalProperty("an-24/set/real_fuel_meter"))
...
-- other pumps
--[[
defineProperty("pump_1", globalProperty("sim/cockpit/engine/fuel_pump_on[0]"))  -- fuel pump for eng 1
defineProperty("pump_2", globalProperty("sim/cockpit/engine/fuel_pump_on[1]"))  -- fuel pump for eng 2
defineProperty("pump_3", globalProperty("sim/cockpit/engine/fuel_pump_on[2]"))  -- fuel pump for eng 3

defineProperty("pump_eng_1", globalProperty("sim/cockpit2/engine/actuators/fuel_pump_on[0]"))  -- fuel pump for eng 1
defineProperty("pump_eng_2", globalProperty("sim/cockpit2/engine/actuators/fuel_pump_on[2]"))  -- fuel pump for eng 2
defineProperty("pump_eng_3", globalProperty("sim/cockpit2/engine/actuators/fuel_pump_on[3]"))  -- fuel pump for eng 3
--]]

defineProperty("real_fuel_pump_1", globalProperty("sim/cockpit2/fuel/fuel_tank_pump_on[0]")) -- real fuel pump for tank 1
defineProperty("real_fuel_pump_4", globalProperty("sim/cockpit2/fuel/fuel_tank_pump_on[3]")) -- real fuel pump for tank 4
...
defineProperty("fuel_pumps_cc", globalProperty("an-24/fuel/fuel_pumps_cc")) -- fuel pumps current consumption
defineProperty("fuel_valves_cc", globalProperty("an-24/fuel/fuel_valves_cc")) -- fuel valves current consumption
defineProperty("fuel_circle_cc", globalProperty("an-24/fuel/fuel_circle_cc")) -- fuel valves current consumption
defineProperty("fuel_meter_cc", globalProperty("an-24/fuel/fuel_meter_cc")) -- fuel meters current consumption
...
function update()
    ...
    if passed > 0 then
        ...
        if get(fuel_block_1) ~= 6 then
            tank1_work = 1
        else
            tank1_work = 0
        end
        if get(fuel_block_2) ~= 6 then
            tank2_work = 1
        else
            tank2_work = 0
        end
        if get(fuel_block_3) ~= 6 then
            tank3_work = 1
        else
            tank3_work = 0
        end
        if get(fuel_block_4) ~= 6 then
            tank4_work = 1
        else
            tank4_work = 0
        end
        ...
        if get(auto_ff) == 1 or fuel_circle * math.abs(tank1 + tank2 - tank3 - tank4) > 100 then
            ...
            local max_num = 0
            if tank1 > tank2 and tank1 > tank3 and tank1 > tank4 and tank1 > 0 then
                max_num = 1
            elseif tank2 > tank3 and tank2 > tank4 and tank2 > 0 then
                max_num = 2
            elseif tank3 > tank4 and tank3 > 0 then
                max_num = 3
            elseif tank4 > 0 then
                max_num = 4
            else
            ...
            if max_num == 1 then
                tank1_q = tank1_q - (engine1_flow + engine2_flow + engine3_flow)
            elseif max_num == 2 then
                tank2_q = tank2_q - (engine1_flow + engine2_flow + engine3_flow)
            elseif max_num == 3 then
                tank3_q = tank3_q - (engine1_flow + engine2_flow + engine3_flow)
            elseif max_num == 4 then
                tank4_q = tank4_q - (engine1_flow + engine2_flow + engine3_flow)
            end
            eng1_use = max_num
            eng2_use = max_num
        else
        ...
        -- XP12 REAL FUEL METER: unusable residual
        -- On a real An-24 each tank has ~50 kg unusable residual
        -- (physically present but not fed to engine — in lower part of tank).
        -- Fuel meter shows LESS than real — pilot accounts for this.
        -- If switch is off — fuel meter shows exactly (for simplification).
        local fuel_meter_real = get(set_real_fuel_meter) == 1
        local unusable = 50 -- kg unusable residual per tank (An-24 AFM)

        -- left wing fuel meter
        if power_27_main * power_115 * get(fuel_q_meter1) > 0 then
            local t1 = tank1_q
            local t2 = tank2_q
            if fuel_meter_real then
                -- show less, but not below 0 (needle will not go past zero)
                t1 = math.max(0, t1 - unusable)
                t2 = math.max(0, t2 - unusable)
            end
            set(tank1_q_ind, t1)
            set(tank2_q_ind, t2)
            fuel_m_cc1 = 1
        ...
        if power_27_main * power_115 * get(fuel_q_meter2) > 0 then
            ...
            local t4 = tank4_q
            if fuel_meter_real then
                t3 = math.max(0, t3 - unusable)
                t4 = math.max(0, t4 - unusable)
            end
            set(tank3_q_ind, t3)
            set(tank4_q_ind, t4)
            fuel_m_cc2 = 1
...
--]]
        ...
        if engine1_flow == 0 or pump1_cc + pump2_cc == 0 then
            set(real_fuel_pump_1, 0)
        else
            set(real_fuel_pump_1, 1)
        end
        if engine2_flow + engine3_flow == 0 or pump3_cc + pump4_cc == 0 then
            set(real_fuel_pump_4, 0)
        else
            set(real_fuel_pump_4, 1)
        end
    end
end