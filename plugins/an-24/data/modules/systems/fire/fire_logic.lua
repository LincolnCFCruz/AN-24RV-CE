...
defineProperty("sim_engine_fire3", globalProperty("sim/cockpit2/annunciators/engine_fires[2]")) -- RU19 engine fire indicator

defineProperty("sim_engine_on_fire1", globalProperty("sim/operation/failures/rel_engfir0")) -- left engine on fire
defineProperty("sim_engine_on_fire2", globalProperty("sim/operation/failures/rel_engfir1")) -- right engine on fire
defineProperty("sim_engine_on_fire3", globalProperty("sim/operation/failures/rel_engfir2")) -- RU19 engine on fire
...
defineProperty("sim_engine_ext2", globalProperty("sim/cockpit2/engine/actuators/fire_extinguisher_on[1]")) -- right engine fire extinguiher
defineProperty("sim_engine_ext3", globalProperty("sim/cockpit2/engine/actuators/fire_extinguisher_on[2]")) -- RU19 engine fire extinguiher

defineProperty("gforce", globalProperty("sim/flightmodel2/misc/gforce_normal")) -- G overload
defineProperty("left_wing_det", globalProperty("sim/operation/failures/rel_wing1L")) -- left wing detach
defineProperty("right_wing_det", globalProperty("sim/operation/failures/rel_wing1R")) -- left wing detach

defineProperty("fire_warinig", globalProperty("an-24/fire/fire_warinig")) -- fire warning for sirene logic
...
function update()
    ...
    if passed > 0 then
        ...
        if get(sim_engine_fire1) == 0 or get(sim_engine_ext1) == 1 then
            left_eng_time = sim_time
        end
        if get(sim_engine_fire2) == 0 or get(sim_engine_ext2) == 1 then
            right_eng_time = sim_time
        end
        if get(sim_engine_fire3) == 0 or get(sim_engine_ext3) == 1 then
            ru19_time = sim_time
        end

        -- reset time for lamps
        if get(sim_engine_fire3) == 1 or get(fire_ru19_btn) == 1 then
            ru19_light_time = sim_time
        end
        if left_wing_fire or get(fire_left_wing_btn) == 1 then
            left_wing_light_time = sim_time
        end
        if right_wing_fire or get(fire_right_wing_btn) == 1 then
            right_wing_light_time = sim_time
        end
        if left_nacelle_fire or get(fire_left_nacelle_btn) == 1 then
            left_nacelle_light_time = sim_time
        end
        if right_nacelle_fire or get(fire_right_nacelle_btn) == 1 then
            right_nacelle_light_time = sim_time
        end
        ...
        if power then
            ...
            end -- right nacelle lamp

            -- ext ready lamps
            if first_turn_ready then
                set(ext_first_ready_lit, 1)
            else
                set(ext_first_ready_lit, 0)
            end
            if second_turn_ready then
                set(ext_second_ready_lit, 1)
            else
                set(ext_second_ready_lit, 0)
            end
            if left_eng_ready then
                set(ext_left_ready_lit, 1)
            else
                set(ext_left_ready_lit, 0)
            end
            if right_eng_ready then
                set(ext_right_ready_lit, 1)
            else
        ...
        -- sirene logic
        if get(fire_main_switcher) == 1 and
            (ru19_light or left_wing_light or right_wing_light or left_nacelle_light or right_nacelle_light or
                get(fire_left_eng_lit) == 1 or get(fire_right_eng_lit) == 1) then
            set(fire_warinig, 1)
        else
            set(fire_warinig, 0)
        end
        ...
        if power and get(fire_main_switcher) > 0 then
            ...
            -- fire timers
            if get(sim_engine_on_fire1) < 6 or not real then
                left_eng_time = sim_time
            end
            if get(sim_engine_on_fire2) < 6 or not real then
                right_eng_time = sim_time
            end
            if get(sim_engine_on_fire3) < 6 or not real then
                ru19_time = sim_time
            end
            ...
            if left_wing_fire then
                left_wing_time = left_wing_time + passed
            end
            if right_wing_fire then
                right_wing_time = right_wing_time + passed
            end

            local G = get(gforce)

            if math.abs(G) > 4.5 - left_wing_time / 200 then
                set(left_wing_det, 6)
            end
            if math.abs(G) > 4.5 - right_wing_time / 200 then
                set(right_wing_det, 6)
            end
        end
    end
end
