defineProperty("siren_flaps", globalProperty("an-24/gauges/flaps_siren"))
defineProperty("siren_auasp", globalProperty("an-24/gauges/auasp_warning"))
defineProperty("siren_ssos", globalProperty("an-24/gauges/ssos_warning"))
defineProperty("siren_fire", globalProperty("an-24/fire/fire_warinig"))
defineProperty("siren_gears", globalProperty("an-24/gauges/gear_siren"))
...
defineProperty("siren_armed", globalProperty("sim/cockpit/weapons/guns_armed"))
defineProperty("frame_time", globalProperty("an-24/time/frame_time")) -- time between frames
...
function update()
    ...
    if button == 1 then
        button_pressed = true
    end -- if button pressed - siren will not work further, till new event come

    if (flaps == 1 or auasp == 1 or ssos == 1 or fire == 1 or gears == 1 or skv_alarm == 1) and not button_pressed then
        if not isSamplePlaying(siren_sound) then
            sasl.al.playSample(siren_sound, true)
        end
    else
        stopSample(siren_sound)
    end

    if flaps ~= flaps_last or gears ~= gars_last or fire ~= fire_last or auasp == 1 or ssos == 1 or skv ~= skv_last then
        button_pressed = false
    end -- if some event come - logical button is released
