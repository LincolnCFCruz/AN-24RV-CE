size = {200, 200}

-- initialize component property table
--  new dataref x12  sim/cockpit2/engine/indicators/EGT_deg_cel -- refresh 18/05/26
--  defineProperty("EGT", globalProperty("sim/cockpit2/engine/indicators/EGT_deg_C[2]"))  -- xp11
defineProperty("EGT", globalProperty("sim/cockpit2/engine/indicators/EGT_deg_cel[2]")) -- xp12

-- needle image
defineProperty("NeedleImage", langImage("needles", 67, 7, 16, 179))

defineProperty("black_cap", langImage("covers", 0, 17, 56, 56)) -- black cap image

local angle = 0

function update()
    local v = get(EGT)
    angle = (v - 200) * 195 / 800 - 110

    -- set limits
    if angle < -110 then
        angle = -117
    elseif angle > 103 then
        angle = 103
    end

    -- -110 = 200
    -- 85 = 1000
end

-- EGT indicator consists of several components
components = { 
    -- white needle
    needle {
        position = {10, 10, 180, 180},
        image = function()
            return get(NeedleImage)
        end,
        angle = function()
            return angle
        end
    }, 
    
    -- black cap
    texture {
        position = {72, 72, 56, 56},
        image = function()
            return get(black_cap)
        end
    }
}
