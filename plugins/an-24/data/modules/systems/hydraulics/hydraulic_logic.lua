...
-- XP12 FIX: additional engine work indicators (N1 in XP12 for TRB_FIX may scale differently)
defineProperty("eng1_N2", globalProperty("sim/flightmodel/engine/ENGN_N2_[0]"))
defineProperty("eng2_N2", globalProperty("sim/flightmodel/engine/ENGN_N2_[1]"))
...
-- gears
defineProperty("gear1", globalProperty("sim/flightmodel2/gear/deploy_ratio[0]")) -- gear deploy ratio. 0 = up, 1 = down
defineProperty("gear2", globalProperty("sim/flightmodel2/gear/deploy_ratio[1]")) -- gear deploy ratio. 0 = up, 1 = down
defineProperty("gear3", globalProperty("sim/flightmodel2/gear/deploy_ratio[2]")) -- gear deploy ratio. 0 = up, 1 = down
defineProperty("emerg_brake", globalProperty("an-24/hydro/emerg_brake")) -- pressure in braking system
defineProperty("steer", globalProperty("sim/flightmodel2/gear/tire_steer_actual_deg[0]")) -- front gear steer deg
defineProperty("frontgear_use_hydro", globalProperty("an-24/hydro/frontgear_use_hydro")) -- front gear steering manipulates by hydraulic system
...
-- defineProperty("brake1", globalProperty("sim/flightmodel/controls/l_brake_add")) -- gear brake ratio. 0 = min, 1 = max
-- defineProperty("brake2", globalProperty("sim/flightmodel/controls/r_brake_add")) -- gear brake ratio. 0 = min, 1 = max

-- wipers
defineProperty("wiper1", globalProperty("sim/flightmodel2/misc/wiper_angle_deg[0]")) -- wiper position in degrees
defineProperty("wiper2", globalProperty("sim/flightmodel2/misc/wiper_angle_deg[1]")) -- wiper position in degrees
defineProperty("wiper_sw", globalProperty("sim/cockpit2/switches/wiper_speed"))

-- flaps
defineProperty("flap1", globalProperty("sim/flightmodel2/wing/flap1_deg[0]")) -- flap deg
defineProperty("flap2", globalProperty("sim/flightmodel2/wing/flap1_deg[1]")) -- flap deg
defineProperty("flap3", globalProperty("sim/flightmodel2/wing/flap2_deg[0]")) -- flap deg
defineProperty("flap4", globalProperty("sim/flightmodel2/wing/flap2_deg[1]")) -- flap deg

-- power calculations
defineProperty("pump_cc", globalProperty("an-24/hydro/pump_cc")) -- emergency pump consumption
...
function update()
    ...
    if passed > 0 then
        ...
        -- XP12 FIX: robust engine work detection.
        -- N1 > 40 was reliable in XP11, but in XP12 may not reach this threshold
        -- for TRB_FIX engines. Combine N1, N2 and is_burning_fuel for safety.
        local left_eng
        if get(eng1_N1) > 40 then
            left_eng = 1
        else
            left_eng = 0
        end
        local right_eng
        if get(eng2_N1) > 40 then
            right_eng = 1
        else
            right_eng = 0
        end
        ...
        emerg_hydro_press = get(emerg_press) + interpolate(hydro_feed_emerg, emerg_hydro_press) * power * power115 *
                                passed * emerg_pump * 3

        -- sound of emerg pump
        if power * power115 * emerg_pump == 1 and pump_gain < 1000 then
            pump_gain = pump_gain + passed * 500
        elseif power * power115 * emerg_pump == 0 and pump_gain > 0 then
            pump_gain = pump_gain - passed * 500
        end
        if pump_gain > 1000 then
            pump_gain = 1000
        elseif pump_gain < 0 then
            pump_gain = 0
        end
        ...
        main_hydro_press = get(hydro_store) +
                               (interpolate(hydro_feed_main, main_hydro_press) * left_eng +
                                   interpolate(hydro_feed_main, main_hydro_press) * right_eng +
                                   interpolate(hydro_feed_emerg, main_hydro_press) * crossfeed) * passed * 3
        ...
        if brake < main_hydro_press and brake < 150 then
            brake = brake + (main_hydro_press - brake) * passed
        end -- calculate pressure for braking system
        ...
        -- flaps
        local flaps_feed = (math.abs(get(flap1) - flap1_last) + math.abs(get(flap2) - flap2_last) +
                               math.abs(get(flap3) - flap3_last) + math.abs(get(flap4) - flap4_last)) * 70

        -- calculate users for buses
        main_hydro_press = main_hydro_press -
                               (gear_feed + brake_feed * (1 - use_emerg) + wipers_feed + nosegear_feed + flaps_feed *
                                   (1 - use_emerg)) * passed
        if main_hydro_press < 40 or get(emerg_brake) > 0 then
            use_emerg = 1
        else
            use_emerg = 0
        end -- when main bus failed - use emergency
        ...
        if main_hydro_press < 10 then
            main_hydro_press = 10
        end
        if emerg_hydro_press < 10 then
            emerg_hydro_press = 10
        end
