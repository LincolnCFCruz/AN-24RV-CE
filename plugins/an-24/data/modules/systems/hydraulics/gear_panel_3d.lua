...
defineProperty("gear2_deploy", globalProperty("sim/aircraft/parts/acf_gear_deploy[2]")) -- deploy of right gear
defineProperty("gear3_deploy", globalProperty("sim/aircraft/parts/acf_gear_deploy[1]")) -- deploy of left gear
defineProperty("gear_force", globalProperty("sim/flightmodel2/gear/tire_vertical_deflection_mtr[0]"))
...
defineProperty("flap_siren", globalProperty("an-24/gauges/flaps_siren"))
defineProperty("gear_siren", globalProperty("an-24/gauges/gear_siren"))
...
-- images
defineProperty("green_led", langImage("gear_lamps", 53, 52, 5, 10))
defineProperty("red_vert_led", langImage("gear_lamps", 42, 44, 5, 10))
defineProperty("red_hor_led", langImage("gear_lamps", 40, 57, 9, 5))
defineProperty("gear_led", langImage("gear_lamps", 0, 43, 36, 21))
defineProperty("flaps_led", langImage("gear_lamps", 0, 21, 36, 21))

local gear_test_button = gPi("an-24/gauges/gear_test_button") -- created in glbl_drfs.lua
...
function update()
    ...
    if get(bus_DC_27_volt_emerg) > 21 then
        ...
        if throttle < 0.24 and not front_green_vis then
            set(gear_siren, 1)
        else
            set(gear_siren, 0)
        end
        if throttle > 0.76 and (flaps < 13 or flaps > 17) and front_green_vis and force ~= 0 then
            set(flap_siren, 1)
        else
            set(flap_siren, 0)
        end

        if front_gear > 0.99 and left_gear > 0.99 and right_gear > 0.99 then
            set(gear_down, 1)
        else
            set(gear_down, 0)
        end
