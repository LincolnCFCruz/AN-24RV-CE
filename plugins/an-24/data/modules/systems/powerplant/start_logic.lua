...
-- custom datarefs
defineProperty("fuel_start1", globalProperty("an-24/start/fuel_start1")) -- fuel valves
defineProperty("fuel_start2", globalProperty("an-24/start/fuel_start2")) -- fuel valves
defineProperty("fuel_start3", globalProperty("an-24/start/fuel_start3")) -- fuel valves
...
defineProperty("ru19_ground_start_cap", globalProperty("an-24/start/ru19_ground_start_btn")) -- start on ground button cap
...
defineProperty("virt_rud1", globalProperty("an-24/misc/virt_rud1"))
defineProperty("virt_rud2", globalProperty("an-24/misc/virt_rud2"))
defineProperty("virt_rud3", globalProperty("an-24/misc/virt_rud3"))
...
defineProperty("tro_need_1", globalProperty("sim/flightmodel/engine/ENGN_thro_use[0]"))
defineProperty("tro_need_2", globalProperty("sim/flightmodel/engine/ENGN_thro_use[1]"))
defineProperty("tro_need_3", globalProperty("sim/flightmodel/engine/ENGN_thro_use[2]"))
...
function update()
    ...
    if passed > 0 then
        ...
        -- V11 (08.06.2026): threshold lowered from 50% to 30% per pilot observation.
        -- In XP12 a prolonged AI-24 start could fail to reach idle within 70 s —
        -- the old protection cut the fuel and the engine died on the first attempt.
        -- The 30% threshold only fires on a genuinely failed start.
        if time_now - left_eng_start_time > 70 and rpm1 < 30 then
            set(fuel_start1, 0)
        end
        -- else set(fuel_start1, 1) end

        if time_now - right_eng_start_time > 70 and rpm2 < 30 then
            set(fuel_start2, 0)
        end
        -- else set(fuel_start2, 1) end

        -- V11: threshold lowered from 40% to 25%. The RU-19 idles at ~30-35% N1 in
        -- XP12 — the old 40% threshold fired 30 s after start and cut the fuel.
        if time_now - ru19_start_time > 30 and rpm3 < 25 then
            set(fuel_start3, 0)
        end
...
--]]
        ...
        if power then
            if selected_eng == -1 and eng_start and
                ((get(virt_rud1) < 0.06 and get(left_prt24_on) == 1 and not eng2_starting) or not real) and
                not start_button_pressed then -- left eng can be started
                eng1_starting = true
                left_eng_start_time = time_now
                start_button_pressed = true
            elseif selected_eng == 1 and eng_start and
                ((get(virt_rud2) < 0.06 and get(right_prt24_on) == 1 and not eng1_starting) or not real) and
                not start_button_pressed then -- right eng can be started
                eng2_starting = true
            ...
            -- XP12 CONFLICT FIX: engine throttle is managed by engine_logic.lua (SASL3,
            -- override_throttles=1). Previously start_logic (SASL2) also wrote throttle here
            -- (0.001/0.075) and conflicted with SASL3 — the engine got stuck at ~40%.
            -- Throttle writes are disabled; everything else (starter/fuel/timings) works.
            ...
            if eng1_starting and not ground_start and not cold_rotate then
                ...
                if time_now - left_eng_start_time > 40 then
                    eng1_starting = false
                end -- finish startup process
            ...
            -- XP12 CONFLICT FIX: throttle write removed (engine_logic.lua SASL3 manages it)
            ...
            if eng2_starting and not ground_start and not cold_rotate then
                ...
                if time_now - right_eng_start_time > 40 then
                    eng2_starting = false
                end -- finish startup process
        ...
        if power or power_em then
            -- V11 ignition fix per the RLE: the An-24 ignition is switched OFF at
            -- the 28th second of the start sequence (from then on the engine runs
            -- on working fuel and the igniter plugs are no longer needed). Ignition
            -- is on only during an active start, per engine independently.
            -- Left engine: ignition on only while starting, up to 28 s
            if eng1_starting and (time_now - left_eng_start_time) < 28 then
                set(sim_igniter1, 1)
                set(sim_ignition1, 1)
            else
                set(sim_igniter1, 0)
            ...
            -- Right engine: same
            if eng2_starting and (time_now - right_eng_start_time) < 28 then
                set(sim_igniter2, 1)
                set(sim_ignition2, 1)
            else
                set(sim_igniter2, 0)
            ...
            -- RU-19 (engine 3) — ignition during its start
            if eng3_starting then
                set(sim_igniter3, 1)
                set(sim_ignition3, 1)
            else
                set(sim_igniter3, 0)
            ...
            -- set lamp indicators
            if eng1_starting or eng2_starting then
                set(apd_work_lit, 1)
            else
                set(apd_work_lit, 0)
            end
        ...
        if start_volt > 70 then
            start_volt = 70
        end
        if start_amp > 600 then
            start_amp = 600
        end
