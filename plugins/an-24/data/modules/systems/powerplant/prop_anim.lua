...
local function checkpark(engn)
    if get(prop_rpm_rad[engn]) < 0.5 and get(engine_park[engn]) == 1 then
        ...
        if not park_angle[ppos][math.floor(angle)] then
            angle = (angle + 0.5) % 360
        else
...
--[[ 
    ...
    function update()
        ...
        -- set disk visible
        if left_rpm > 300 then 
            set(prop_is_disc_1, 1) 
        else set(prop_is_disc_1, 0) 
        end
        if right_rpm > 300 then 
            set(prop_is_disc_2, 1) 
        else 
            set(prop_is_disc_2, 0) 
        end
        ...
        if left_prop_angle > 360 then 
            left_prop_angle = left_prop_angle - 360 
        end
        if right_prop_angle > 360 then 
            right_prop_angle = right_prop_angle - 360 
        end

        -- set limits
        local left_angle = left_prop_angle
        local right_angle = right_prop_angle
        while left_angle > 360 do 
            left_angle = left_angle - 360 
        end
        while right_angle > 360 do 
