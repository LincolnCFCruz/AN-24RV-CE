...
defineProperty("sim_oil_q1", globalProperty("sim/cockpit2/engine/indicators/oil_quantity_ratio[0]")) -- quantity ratio
defineProperty("sim_oil_q2", globalProperty("sim/cockpit2/engine/indicators/oil_quantity_ratio[1]")) -- quantity ratio
...
function update()
    ...
    if dcOK() then
        ...
        if left_angle > 126 then
            set(oil_lamp1, 1)
        end

        if right_angle < -126 then
            set(oil_lamp2, 1)
        end
    else
        left_angle = 150
    ...
    if left_angle > 150 then
        left_angle = 150
    end

    if right_angle < -150 then
        right_angle = -150
    end
end
