...
defineProperty("frequence", globalProperty("sim/cockpit2/radios/actuators/com2_frequency_hz"))
...
defineProperty("emerg1", globalProperty("sim/flightmodel2/misc/custom_slider_ratio[8]")) -- open/close emerg_hatch1
defineProperty("emerg2", globalProperty("sim/flightmodel2/misc/custom_slider_ratio[9]")) -- open/close emerg_hatch2
defineProperty("cargo1", globalProperty("sim/flightmodel2/misc/custom_slider_ratio[4]")) -- open/close cargo_hatch1
defineProperty("cargo2", globalProperty("sim/flightmodel2/misc/custom_slider_ratio[6]")) -- open/close cargo_hatch2
defineProperty("paxdoor", globalProperty("sim/flightmodel2/misc/custom_slider_ratio[5]")) -- open/close pax_door
defineProperty("time_sim1", globalProperty("sim/time/total_running_time_sec"))
...
-- 1 - first greeting, 2 - radio power off (plain hiss), 3 - reminder that something is not closed / not removed
-- 4 - GPU request, 5 - GPU disconnect, 6 - repeated greeting, 7 - final goodbye
local energy = 0
local check = 0
local check2 = 0
local check3 = 0
...
function update()
    ...
    if get(time_sim1) > cur_time then
        if get(GS) < 2 then
            if get(emerg1) == 0 and get(emerg2) == 0 and get(cargo1) == 0 and get(cargo2) == 0 and get(paxdoor) == 0 and
                get(ground_available) == 0 and get(left_eng_main) == 1 and get(left_eng_ext) == 1 and get(right_eng_ext) ==
                1 and get(right_eng_main) == 1 and get(ru19_eng_ext) == 1 and get(antiice_left) == 1 and
                get(antiice_right) == 1 and get(rockets) == 1 and get(pitot_1) == 1 and get(pitot_2) == 1 and
                get(pitot_3) == 1 and get(grounding) == 1 and get(gear_blocks) == 1 then
                checktrue = 1
            else
                checktrue = 0
            end
            ...
            if get(frequence) == 13180 and energy == 0 and get(service_subpanel) == 0 and get(ground_sound) ~= 4 and
                get(ground_sound) ~= 5 then
                set(ground_sound, 2)
            end
            ...
            if get(frequence) == 13180 and energy == 0 and check == 1 and get(service_subpanel) == 1 and
                get(ground_sound) ~= 4 and get(ground_sound) ~= 5 then
                set(ground_sound, 2)
                set(service_subpanel, 0)
            end

            if get(frequence) ~= 13180 and energy == 1 and checktrue == 1 and check == 1 and get(service_subpanel) == 1 then
                set(service_subpanel, 0)
                set(ground_sound, 7)
                check = 0
            end

            if get(frequence) ~= 13180 and energy == 1 and checktrue == 0 and check == 1 and check2 == 0 and
                get(service_subpanel) == 1 then
                set(service_subpanel, 0)
                set(ground_sound, 3)
                check2 = 1
            end
