...
function update() 
    ...
    if passed > 0 then
        ...
        if get(ground_available) == 1 and get(power_mode) == 0 then
            ground_power = 1
        end
        ...
        if bat_on1 == 1 or bat_on2 == 1 or bat_on3 == 1 then -- all of calculations can be skipped if batteries are off
            ...
            if get(stg1_is_gen) * get(stg1_on_bus) == 0 and get(stg2_is_gen) * get(stg2_on_bus) == 0 and
                get(gs24_is_gen) * get(gs24_on_bus) == 0 and ground_power == 0 then
                set(bat_amp_cc, 0)
                bat_source = true
            else
                set(bat_amp_cc,
                    (28 * (bat_on1 + bat_on2 + bat_on3) - bat1_capacity * bat_on1 - bat2_capacity * bat_on2 -
                        bat3_capacity * bat_on3)) -- bat current depends on their charge
                bat_source = false
            end
