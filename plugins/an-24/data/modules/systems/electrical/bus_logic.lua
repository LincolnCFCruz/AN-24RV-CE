...
defineProperty("sim_bat_on", globalProperty("sim/cockpit/electrical/battery_on"))
defineProperty("sim_avionics_on", globalProperty("sim/cockpit/electrical/avionics_on"))

defineProperty("sim_inv1_on", globalProperty("sim/cockpit2/electrical/inverter_on[0]"))
defineProperty("sim_inv2_on", globalProperty("sim/cockpit2/electrical/inverter_on[1]"))
...
function update()
    ...
    if get(power_mode) == 2 then
        if get(bat1_on) == 1 or get(bat2_on) == 1 or get(bat3_on) == 1 then
            bats_on = 1
        else
            bats_on = 0
        end -- define if batteries are ON
...
--]]
    ...
    if DC27_source == 4 then
        emerg_volt = get(bat_all_volt) * bats_on
    else
        emerg_volt = DC27_volt
    end -- emergency bus works as long as batteries are connected
    ...
    if get(inv_PT1000_1) == 1 and (DC27_volt > 21 or emerg_volt > 21) then
        AC36_volt = 36
    elseif get(inv_PT1000_2) == 1 and emerg_volt > 21 then
        AC36_volt = 36
    else
        AC36_volt = 0
    end
    ...
    -- local PT750_amp = 0
    if AC115_source == 1 then
        set(go1_amp, get(bus_AC_115_amp))
    else
        set(go1_amp, 0)
    end -- current on GO-left
    if AC115_source == 2 then
        set(go2_amp, get(bus_AC_115_amp))
    else
        set(go2_amp, 0)
    end

--[[
    -- if AC115_source == 3 then 
        PT750_amp = get(bus_AC_115_amp) 
    end -- calculate current on PT750
--]]

    local bus27_amp = get(bus_DC_27_amp) -- + get(bus_DC_27_amp_emerg)   -- current on main bus will be calculated in other logic

--[[
    if DC27_source == 1 then 
        set(stg1_amp, bus27_amp) 
    else 
        set(stg1_amp, 0) 
    end  -- current on STG-left

    if DC27_source == 2 then 
        set(stg2_amp, bus27_amp) 
    else 
        set(stg2_amp, 0) 
    end -- current on STG-right
...
--]]
    ...
    if DC27_source == 4 then
        ...
        set(bat_all_amp, 0 - get(bat_amp_cc))
    end
