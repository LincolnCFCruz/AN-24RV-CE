...
-- sim load
defineProperty("payload", globalProperty("sim/flightmodel/weight/m_fixed"))  -- payload weight, kg
defineProperty("CG_load", globalProperty("sim/flightmodel/misc/cgz_ref_to_default")) -- Center of Gravity reference to default, m
...
-- images
defineProperty("vert_plank", sasl.gl.loadImage("payload_panel_e.dds", 719, 919, 2, 100))
defineProperty("hor_plank_green", sasl.gl.loadImage("payload_panel_e.dds", 730, 1020, 100, 2))
defineProperty("hor_plank_orange", sasl.gl.loadImage("payload_panel_e.dds", 730, 1014, 100, 2))
defineProperty("red_flag", sasl.gl.loadImage("payload_panel_e.dds", 734, 977, 70, 25))
-- digits images
defineProperty("digitsImage", sasl.gl.loadImage("black_digit_strip.png", 1, 60, 14, 196))



local extview = globalProperty("sim/graphics/view/view_is_external")
local crew = globalProperty("an-24/anim/crew")
...
-- Weight & balance digit readout: builds a digitstape with the shared
-- image/allowNonRound defaults; position, digit count, value and the optional
-- showLeadingZeros / fractional fields vary per call.
local function wbDigit(pos, n, val, lead, frac)
    local d = { position = pos, image = digitsImage, digits = n, allowNonRound = true, value = val }
    if lead ~= nil then d.showLeadingZeros = lead end
    if frac ~= nil then d.fractional = frac end
    return digitstape(d)
end
...
components = {
  ...
  -- background
  textureLit {
    image = langImage("payload_panel", 5, 31, size[1], size[2]),
  ...
  ---------------------
  -- empty airplane_CG
   wbDigit({ 595, 846, 40, 15}, 3, 22, false, 0);
  -- empty plane weight
   wbDigit({ 283, 831, 67, 15}, 5, function() return empty_plane_weight end);

  -- rutkit weight
   wbDigit({ 283, 817, 67, 15}, 5, function() return rutkit_weight end);

  -- prepared_plane_weightt
   wbDigit({ 283, 803, 67, 15}, 5, function() return prepared_plane_weight end);
  -- prepared_plane_weightt
   wbDigit({ 400, 803, 72, 15}, 5, function() return prepared_plane_weight end);

  -- crew_weight
   wbDigit({ 400, 789, 72, 15}, 5, function() return crew_weight end);

  -- stuard_weight
   wbDigit({ 400, 776, 72, 15}, 5, function() return stuard_weight end);

  -- fuel_weight
   wbDigit({ 400, 762, 72, 15}, 5, function() return fuel_weight end);

  -- loaded_plane_weight
   wbDigit({ 400, 748, 72, 15}, 5, function() return loaded_plane_weight end);
   wbDigit({ 619, 748, 75, 15}, 5, function() return loaded_plane_weight end);

  -- max_plane_weight
   wbDigit({ 619, 762, 75, 15}, 5, function() return max_plane_weight end);

  -- max_payload
   wbDigit({ 619, 733, 75, 15}, 5, function() return max_payload end);

  -- prepared_plane_weightt
   wbDigit({ 619, 668, 75, 18}, 5, function() return prepared_plane_weight end);

  -- crew_weight
   wbDigit({ 619, 643, 75, 18}, 5, function() return crew_weight end);

  -- stuard_weight
   wbDigit({ 619, 620, 75, 18}, 5, function() return stuard_weight end);

  -- all_pax_mass
   wbDigit({ 619, 575, 75, 18}, 5, function() return all_pax_mass end);

  -- row_1_mass
   wbDigit({ 619, 551, 75, 18}, 5, function() return row_1_mass end);

  -- row_2_mass
   wbDigit({ 619, 528, 75, 18}, 5, function() return row_2_mass end);

  -- row_3_mass
   wbDigit({ 619, 505, 75, 18}, 5, function() return row_3_mass end);

  -- row_4_mass
   wbDigit({ 619, 483, 75, 18}, 5, function() return row_4_mass end);

  -- row_5_mass
   wbDigit({ 619, 461, 75, 18}, 5, function() return row_5_mass end);

  -- row_6_mass
   wbDigit({ 619, 438, 75, 18}, 5, function() return row_6_mass end);

  -- cargo_1
   wbDigit({ 619, 415, 75, 18}, 5, function() return cargo_1 end);

  -- cargo_2
   wbDigit({ 619, 392, 75, 18}, 5, function() return cargo_2 end);

  -- cargo_3
   wbDigit({ 619, 368, 75, 18}, 5, function() return cargo_3 end);

  -- fuel_weight
   wbDigit({ 619, 345, 75, 18}, 5, function() return fuel_weight end);

  -- airplane_weight
   wbDigit({ 619, 305, 75, 18}, 5, function() return airplane_weight end);
  ...
  -- crew_weight
   wbDigit({ 200, 643, 15, 18}, 1, function() return crew_weight / 80 end, false);

  -- stuard_weight
   wbDigit({ 195, 620, 35, 18}, 3, function() return stuard_weight end);

  -- all_pax_mass
   wbDigit({ 197, 575, 25, 18}, 2, function() return all_pax_mass / 75 end);

  -- row_1_mass
   wbDigit({ 200, 551, 15, 18}, 1, function() return row_1_mass / 75 end);

  -- row_2_mass
   wbDigit({ 200, 528, 15, 18}, 1, function() return row_2_mass / 75 end);

  -- row_3_mass
   wbDigit({ 200, 505, 15, 18}, 1, function() return row_3_mass / 75 end);

  -- row_4_mass
   wbDigit({ 200, 483, 15, 18}, 1, function() return row_4_mass / 75 end);

  -- row_5_mass
   wbDigit({ 200, 461, 15, 18}, 1, function() return row_5_mass / 75 end);

  -- row_6_mass
   wbDigit({ 200, 438, 15, 18}, 1, function() return row_6_mass / 75 end);

  -- cargo_1
   wbDigit({ 195, 415, 35, 18}, 4, function() return cargo_1 end);

  -- cargo_2
   wbDigit({ 195, 392, 30, 18}, 3, function() return cargo_2 end);

  -- cargo_3
   wbDigit({ 195, 368, 30, 18}, 3, function() return cargo_3 end);



  -- loaded_plane_weight
   wbDigit({ 249, 142, 75, 15}, 5, function() return loaded_plane_weight end);

  -- comercial_payload
   wbDigit({ 249, 129, 75, 15}, 5, function() return comercial_payload end);

  -- airplane_weight
   wbDigit({ 249, 115, 75, 15}, 5, function() return airplane_weight end);

  -- airplane_CG
   wbDigit({ 335, 115, 50, 15}, 3, function() return to_CG end, false, 1);

  -- airplane_CG with gears retracted
   wbDigit({ 430, 115, 50, 15}, 3, function() return to_gear_CG end, false, 1);


  -- used_fuel
   wbDigit({ 249, 101, 75, 15}, 5, function() return used_fuel end);

  -- landing_weight
   wbDigit({ 249, 87, 75, 15}, 5, function() return landing_weight end);

  -- airplane_CG
   wbDigit({ 335, 87, 50, 15}, 3, function() return lan_CG end, false, 1);

  -- airplane_CG
   wbDigit({ 430, 87, 50, 15}, 3, function() return lan_gear_CG end, false, 1);

  -- flight_distance
   wbDigit({ 249, 61, 75, 15}, 5, function() return flight_distance end);

  -- reserv_distance
   wbDigit({ 249, 47, 75, 15}, 5, function() return reserv_distance end);

  -- taxi_time
   wbDigit({ 249, 33, 75, 15}, 5, function() return taxi_time end);

  -- fuel_stock
   wbDigit({ 249, 19, 75, 15}, 5, function() return fuel_stock end);
