...
defineProperty("roll_spd", globalProperty("sim/flightmodel/position/P")) -- current roll speed deg/sec
defineProperty("pitch_spd", globalProperty("sim/flightmodel/position/Q")) -- current pitch speed deg/sec
...
defineProperty("main_press", globalProperty("an-24/hydro/main_press")) -- pressure in main system. initial 120 kg per square cm. maximum 160.
...
local ail_pos_need = 0 -- needed aileron position
...
local pitch_clar = 0.01 -- clarify the pitch change speed
...
function update()
    ...
    local roll_spd_need = roll_diff * roll_spd_lim -- roll speed for reaching the needed position
    ...
    if roll_spd_need > roll_spd_now + roll_clar then
        ail_pos_need = ail_pos_now + ail_step * passed
    elseif roll_spd_need < roll_spd_now - roll_clar then
        ail_pos_need = ail_pos_now - ail_step * passed
    end

    if ail_pos_need > 0.5 then
        ail_pos_need = 0.5
    elseif ail_pos_need < -0.5 then
        ail_pos_need = -0.5
    end
    ...
    if roll_power and press then
        ...
        if ail_pos_need > 0 then
            ail_pos_need = ail_pos_need * 0.9
        elseif ail_pos_need < 0 then
            ail_pos_need = ail_pos_need * 0.9
        end
        set(ap_roll_comm, ail_pos_need)
    end
    ...
    local pitch_diff = get(ap_pitch_diff) / pitch_norm_diff -- pitch speed for reaching the needed position
    ...
    if pitch_spd_need > pitch_spd_now + pitch_clar then
        elev_pos_need = elev_pos_now + elev_step * passed
    elseif pitch_spd_need < pitch_spd_now - pitch_clar then
        elev_pos_need = elev_pos_now - elev_step * passed
    end

    if elev_pos_need > 0.5 then
        elev_pos_need = 0.5
    elseif elev_pos_need < -0.5 then
        elev_pos_need = -0.5
    end
    ...
    if pitch_power and press then
        ...
        if elev_pos_need > 0 then
            elev_pos_need = elev_pos_need * 0.9
        elseif elev_pos_need < 0 then
            elev_pos_need = elev_pos_need * 0.9
        end
        set(ap_pitch_comm, elev_pos_need)
    end
    ...
    local hdg_diff = get(ap_hdg_diff) / hdg_norm_diff -- hdg speed for reaching the needed position
    if hdg_diff > 1 then
        hdg_diff = 1
    elseif hdg_diff < -1 then
        hdg_diff = -1
    end
    ...
    if hdg_spd_need < hdg_spd_now + hdg_clar then
        rudd_pos_need = rudd_pos_now + rudd_step * passed
    elseif hdg_spd_need > hdg_spd_now - hdg_clar then
        rudd_pos_need = rudd_pos_now - rudd_step * passed
    end

    if rudd_pos_need > 0.3 then
        rudd_pos_need = 0.3
    elseif rudd_pos_need < -0.3 then
        rudd_pos_need = -0.3
    end
    ...
    if hdg_power and press then
        ...
        if rudd_pos_need > 0 then
            rudd_pos_need = rudd_pos_need * 0.9
        elseif rudd_pos_need < 0 then
            rudd_pos_need = rudd_pos_need * 0.9
        end
        set(ap_yaw_comm, rudd_pos_need)
    end
